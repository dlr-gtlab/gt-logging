...
SET SERVERPATH=C:\deployment\%PLATFORMNAME%\%CI_PROJECT_NAME%\%CI_COMMIT_TAG%
...
if %PLATFORMNAME%==linux (
    for /r .\lib %%f in (*.so) do @copy "%%f" %SERVERPATH%\lib\%TARGETDIRNAME%\
    for /r .\lib %%f in (*-d.so) do @copy "%%f" %SERVERPATH%\%DEBUGBINDIRECTORY%\
    for /r .\lib %%i in (*.so) do (echo %%i | findstr /r ".*-d.so" > nul || copy %%i %SERVERPATH%\%BINDIRECTORY%\)
)
